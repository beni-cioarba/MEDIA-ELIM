...
| **3 · Web compacta y profesional** | ✅ **Fundamentos hechos (22/09/2026)**: radios, elevación, raíz tipográfica, escala, botones planos, densidad por tokens, superficie oscura, catálogo `ui-*` y guía viva `/stil` (`47-design-language.md`) · ⏳ migrar el marcado pantalla a pantalla a las primitivas (home, galería, transmisiones, cabeceras de sección) y borrar el CSS que sustituyen | 1–2 días restantes | Criterios de `design-posture` + snippet responsive `ok`; el usuario valida el aspecto |
